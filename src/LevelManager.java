...
public class LevelManager extends ViewManager
{
    ...
    {
        this(800, 800, tileSize, levelData);
    }

    // Constructor
    public LevelManager(int width, int height, int tileSize, ELEMENT[][] levelData)
    {
        super(width, height);
        this.pane.setStyle("-fx-background-color: #1f1f1f");

        this.level = new LevelScene(tileSize, levelData);
        this.level.addSubSceneToPane(pane);
        ...
        this.player.addToPane(level.getPane(), 2, 2, tileSize);
    ...
    {
        for(int i=0; i<level.getCollidableElements().size(); i++)
        {
            if(player.getCollisionBox().getBoundsInParent().intersects(level.getCollidableElements().get(i).getBoundsInParent()))
            {
                return true;
            }
        }
    ...
    {
        level.getScene().setLayoutX((pane.getWidth()/2) - player.getCollisionBox().getLayoutX());
        level.getScene().setLayoutY((pane.getHeight()/2) - player.getCollisionBox().getLayoutY());
    }
    ...
    private void createPaneResizeListeners()
    {
        pane.heightProperty().addListener(new ChangeListener()
        {
            ...
            public void changed(ObservableValue obersvable, Object oldValue, Object newValue)
            {
                ...
                level.getScene().setLayoutY(level.getScene().getLayoutY() + deltaHeight/2);
            }
        });
        ...
        pane.widthProperty().addListener(new ChangeListener() {
            ...
            public void changed(ObservableValue obersvable, Object oldValue, Object newValue)
            {
                ...
                level.getScene().setLayoutX(level.getScene().getLayoutX() + deltaWidth/2);
            }
        });
    }
}
